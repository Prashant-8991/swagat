    ...
    <>
      <div className="fixed bottom-8 right-8 z-40">
        <button
          ...
          className={`
            h-14 w-14 rounded-full shadow-xl shadow-indigo-500/30 flex items-center justify-center transition-all duration-300
            hover:scale-110 active:scale-95 bg-indigo-600 hover:bg-indigo-700 text-white
          `}
          aria-label="Open Global Filters"
        >
          <HiFunnel className="w-6 h-6" />
          {hasActiveFilters && (
            <span className="absolute -top-1 -right-1 h-5 w-5 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center border-2 border-white dark:border-gray-900 shadow-sm animate-pulse">
              {activeFilterCount}
            </span>
          )}
        </button>
