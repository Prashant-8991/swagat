...
export default function TalukaTreeMap({
    ...
    const sortedData = useMemo(() => {
        const filtered = data.filter((d) => d.taluka && d.taluka.trim() !== "" && d.taluka.toLowerCase() !== "null");
        ...
        >
            ...
            <div className="flex w-full h-full gap-2 overflow-hidden pt-10">
                {columns.map((colItems, colIdx) => (
                    ...
                    >
                        {colItems.map((item) => {
                            const globalIndex = sortedData.findIndex(d => d.taluka === item.taluka);
                            const color = currentPalette[globalIndex % currentPalette.length];
                            const isActive = hoveredData?.taluka === item.taluka;

                            return (
                                <motion.div
                                    key={item.taluka}
                                    layoutId={item.taluka}
                                    onClick={() => item.taluka && onTileClick && onTileClick(item.taluka)}
                                    onContextMenu={(e) => {
                                        e.preventDefault();
                                        if (onContextMenu) {
                                            onContextMenu({
