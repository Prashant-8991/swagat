...
interface StickyMiniActiveFiltersProps {
 ...
 convertFilterToDisplay: (monthFilter: string) => string;
}
...
export default function StickyMiniActiveFilters({
 ...
 useEffect(() => {
 const activeFiltersPane = document.getElementById('active-filters-pane');
 if (!activeFiltersPane) {
 setShowSticky(false);
 return;
 }
 ...
 observer.observe(activeFiltersPane);
 ...
 if (!showSticky || !hasActiveFilters) {
 return null;
 }
 ...
 >
 <div className="pointer-events-auto max-w-[600px] w-full bg-white/60 backdrop-blur-lg dark:bg-gray-900/60 border border-gray-200/40 dark:border-gray-700 rounded-lg px-4 py-2 flex items-center gap-2 shadow-sm">
 ...
 <span key={dept} className="px-3 py-1.5 bg-slate-500 text-white rounded-lg text-xs flex items-center gap-2 shadow-sm">
 Department: <strong>{dept}</strong>
 <button onClick={() => clearGlobalFilter('departments')} className="hover:text-slate-100 ml-1">✕</button>
 </span>
 ))}
 ...
 <span key={type} className="px-3 py-1.5 bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 rounded-lg text-xs flex items-center gap-2 border border-teal-200 dark:border-teal-800">
 Program: <strong>{type}</strong>
 <button
 onClick={() => {
 const newProgramTypes = filters.programTypes!.filter(t => t !== type);
 ...
 aria-label="Clear Taluka filter"
 >
 ✕
 </button>
 </span>
 ...
 <span className="px-3 py-1.5 bg-slate-100 dark:bg-slate-900/30 text-slate-700 dark:text-slate-300 rounded-lg text-xs flex items-center gap-2 border border-slate-200 dark:border-slate-800">
 ...
 AI Category: <strong>{filters.aiCategory}</strong>
 <button
 onClick={() => updateFilter('aiCategory', null)}
 className="hover:text-teal-900 dark:hover:text-teal-100 ml-1"
 aria-label="Clear AI Category filter"
 >
 ✕
 </button>
 </span>
 ...
 Month: <strong>{convertFilterToDisplay(filters.month)}</strong>
 <button
 ...
 <span className="px-3 py-1.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-lg text-xs flex items-center gap-2 border border-amber-200 dark:border-amber-800">
 ...
 aria-label="Clear Sub Status filter"
 >
 ✕
 </button>
 </span>
 ...
 className="ml-auto px-2 py-1 rounded bg-red-50 hover:bg-red-100 dark:bg-red-900/30 dark:hover:bg-red-900/50 text-red-600 dark:text-red-400 text-xs font-medium border border-red-200 dark:border-red-800 shadow-sm transition-all"
